...
const GameCanvas: React.FC<GameCanvasProps> = ({
    ...
    const drawGL = (texId: string | null, x: number, y: number, w: number, h: number, angle: number, alpha: number, ux = 0, uy = 0, uw = 1, uh = 1, color = [1, 1, 1, 1]) => {
        ...
        if (locs.time) gameGl.uniform1f(locs.time, frameCountRef.current);
    ...
    useEffect(() => {
        ...
        const gameLoop = () => {
            ...
            // 1. Update Game Engine (Execution Pipeline)
            if (engineRef.current) {
                engineRef.current.update();
            }

            // 2. Render Game to Canvas
            if (rendererRef.current && engineRef.current) {
                if (rendererRef.current.textures !== texturesRef.current) {
                    rendererRef.current.textures = texturesRef.current;
                }
                rendererRef.current.render(engineRef.current.state, performance.now());
            }
    ...
    useEffect(() => {
        ...
        const fsSource = `
            ...
            void main() {
                vec4 tex = vec4(1.0);
                if (u_useTexture > 0.5) {
                    tex = texture2D(u_image, v_texCoord);
                }

                vec4 color = tex * u_color;

                // Simple Dynamic Lightning Simulation (Path Mode style)
                vec2 center = gl_FragCoord.xy / u_resolution - 0.5;
                float vignette = 1.0 - dot(center, center) * 0.8;
                vignette = pow(vignette, 1.2);

                // Add a subtle glow/pulsation to the color
                float pulse = sin(u_time * 0.005) * 0.05 + 1.0;
                color.rgb *= pulse;
        ...
        glLocsRef.current = {
            ...
            time: gameGl.getUniformLocation(program, 'u_time')
        };
